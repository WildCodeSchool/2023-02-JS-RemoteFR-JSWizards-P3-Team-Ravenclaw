  ...
  return (
    <aside className="hidden md:flex md:flex-col">
      <nav className="relative h-full min-h-[calc(100vh-80px)] w-11/12 bg-neutralDarkest p-5 pt-8 text-xl font-medium duration-300 md:w-72">
          ...
          {menus.map((menu) => {
            return (
              <li key={menu.id} className={`${styles.dashList}`}>
                <NavLink
                  to={menu?.path}
                  className={`flex w-full cursor-pointer pl-4 text-lg decoration-primaryLightest hover:text-primaryLightest md:rounded-lg
                      md:hover:bg-black md:active:text-primaryLightest dark:md:hover:bg-white`}
                >
                  {menu.title}
                </NavLink>
              </li>
        ...
        <div className="group flex cursor-pointer items-center gap-4 pl-4">
          <svg
            viewBox="0 0 64 64"
            fill="none"
          ...
          >
            <g clipPath="url(#clip0_113_1997)">
              <path
                ...
                className="fill-neutralLightest group-hover:fill-primaryLightest"
              />
              <path
                fillRule="evenodd"
                clipRule="evenodd"
          ...
          <button
            type="button"
            className="text-lg group-hover:text-primaryLightest"
            onClick={handleClick}
          >
            Log Out
          </button>
